...
✅ Uses pyzbar (ZBar) for QR decode instead of cv2.QRCodeDetector
✅ Still uses OpenCV for decode pipeline support (grayscale conversion, preview, resize, video, etc.)
...
class QRCamera:
    ...
    def __init__(
        ...
        prefer_largest: bool = True,     # pick largest QR in frame (recommended)
    ):
    ...
    def start(self) -> bool:
        ...
        log.info("🎥 QRCamera started (capture 2028×1520, display half, pyzbar)")
        return True
    ...
    def _pick_code(self, codes: List[Any]) -> Any:
        if not codes:
            return None
        if not self.prefer_largest:
            return codes[0]
        return max(codes, key=lambda c: int(c.rect.width) * int(c.rect.height))
    ...
    def _run_qr(self, frame: np.ndarray) -> Optional[Dict[str, Any]]:
        ...
        payload = ""
        try:
            if getattr(code, "data", None):
                payload = code.data.decode("utf-8", errors="ignore").strip()
        except Exception:
            payload = ""
